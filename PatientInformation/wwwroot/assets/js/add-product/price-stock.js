...
const PopulatePriceStockTable = e => {
    ...
    if (ValidateVariationOption()) {
        ...
        fetch(`/Product/PriceAndStockFormPartial`, {
            method: 'post',
    ...
    document.querySelectorAll('.input-price-stock').forEach(i => {
        ...
        document.querySelectorAll(`.td-cell[data-col='${column}'] input${(column === '5' ? '.input-sale-price' : '')}`).forEach(j => {
            ...
            if ((column === '2' || column === '3') && value !== '') {
                ...
                if (ind >= 26 && ind < 52) {
                    ind = ind - 26;
                    j.value = `${value}-${varNum}a${alpha[ind]}`;
                }
                if (ind >= 52 && ind < 78) {
                    ind = ind - 52;
                    j.value = `${value}-${varNum}b${alpha[ind]}`;
                }
                if (ind >= 78 && ind < 104) {
                    ind = ind - 78;
                    j.value = `${value}-${varNum}c${alpha[ind]}`;
                }
                if (ind >= 104 && ind < 130) {
                    ind = ind - 104;
                    j.value = `${value}-${varNum}d${alpha[ind]}`;
                }
                if (ind >= 130 && ind < 156) {
                    ind = ind - 130;
                    j.value = `${value}-${varNum}e${alpha[ind]}`;
                }
            }
...
$(document).on('change', '.quantity-input', function (e) {
    ...
    if ($.isNumeric(inputVal)) {
        ...
        didRangeValidationFail = true;
        $(this).val('');
        if ($parent.hasClass("cell-error") == false && $parent.hasClass("cell-quantity")) {
            $parent.addClass("cell-error");
        }
    }
});
